...
curl -X 'POST' --silent --data-binary '{"text":"A new build for the gameon board service has started."}' $WEBHOOK > /dev/null
...
then
  ...
  then
    ...
    curl -X 'POST' --silent --data-binary '{"text":"Docker build for the gameon board service has failed."}' $SLACK_WEBHOOK_PATH > /dev/null
    exit -2
  else
    ...
    then
      ...
      curl -X 'POST' --silent --data-binary '{"text":"Docker run for the gameon board service has failed."}' $SLACK_WEBHOOK_PATH > /dev/null
      exit -3
    else
      cd ..
      rm -rf dockercfg
